(() => {
  ...
  const aliases = {
    ...
    city:["city"], state:["state"], zip:["zip","zipcode","postalcode"],
    phone:["phone","phonenumber","mobile","cell","cellphone","telephone"],
    email:["email","emailaddress"], asking_price:["askingprice","price","amount","offerprice"],
    notes:["notes","note","comments","comment","details","description"]
  };
  let client, user, existing = [], reviewed = [];
  ...
  function normalizedRows(rawRows) {
    ...
    return rawRows.map((raw, index) => {
      const asking = readField(raw, "asking_price").replace(/[^0-9.-]/g, "");
      const row = {
        seller_name: readField(raw, "seller_name"), property_address: readField(raw, "property_address"),
        city: readField(raw, "city"), state: (readField(raw, "state") || "AL").toUpperCase(), zip: readField(raw, "zip"),
        phone: readField(raw, "phone"), email: readField(raw, "email").toLowerCase(), asking_price: asking ? Number(asking) : null,
  ...
  async function review() {
    ...
    if (file.size > 3 * 1024 * 1024) { message.textContent = "Use a CSV smaller than 3 MB."; return; }
    const rawRows = parseCsv(await file.text());
    if (!rawRows.length) { message.textContent = "No usable data was found. Make sure it is a CSV with a header row."; return; }
  ...
  async function importRows() {
    ...
    try {
      for (let i = 0; i < rows.length; i += 100) {
        const { error } = await client.from("leads").insert(rows.slice(i, i + 100));
        if (error) throw error;
        done += Math.min(100, rows.length - i); message.textContent = done + " of " + rows.length + " leads imported…";
      }
    ...
    } catch (error) { message.textContent = "Import stopped: " + (error.message || "Could not add the leads.") + ". Rows already added were kept."; }
  ...
  async function initialize(event) {
    ...
    client = event.detail.client; user = event.detail.user;
