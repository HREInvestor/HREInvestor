...
Deno.serve(async request=>{
 ...
 const payload=await response.json(); if(!response.ok){const detail=[payload?.message,payload?.error,Array.isArray(payload?.errors)?payload.errors.map((item:any)=>typeof item==="string"?item:item?.message||item?.detail).filter(Boolean).join("; "):""].find((item:any)=>typeof item==="string"&&item.trim()); return new Response(JSON.stringify({error:detail?`SignWell (${response.status}): ${detail.slice(0,500)}`:`SignWell request failed (${response.status}).`}),{status:502,headers:cors});}
